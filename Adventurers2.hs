...
fun' :: (Int -> Bool) -> Duration State -> Bool
fun' f (Duration (i, x)) = (f i) && (and (map x players))
...
-- False                False                 False
-- > test 5 (<17)       > test 5 (<=17)       > test 5 (<17)
-- False                True                  True
-- > test 6 (<17)       > test 6 (<=17)       > test 6 (<17)
-- False                True                  True
...
-- previous duration states, [Duration State]. 
data Duration2 a = Duration2 (Int, a, [Duration State]) deriving Show
...
getStates2 :: Duration2 a -> [Duration State]
...
fun4 s (h:t) = let s' = mChangeState ((h:t)++[(Right ())]) s
                   i  = getTimeAdv((\(Left a) -> a) h)
               in Duration2 (i, s', [Duration (i,s)])
...
propagate2 0 _   s = LD2 [Duration2 (0, s, [Duration (0, s)])]
...
-- 'Duration State' that satisfies the previous conditions, and 'l' is a list of 'Duration2 State'
